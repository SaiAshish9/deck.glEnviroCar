...
const data = [{sourcePosition: [6.48471,51.225467], targetPosition: [-122.41669, 37.781]}];
...
import GeoJsonData from './abcd.geojson'
...
class SimpleZipMap extends React.Component {
  ...
  componentDidMount() {
    const data = fetchJSON(
      ...
      // 'https://envirocar.org/api/stable/measurements/'
// GeoJsonData
'https://raw.githubusercontent.com/SaiAshish9/deck.glEnviroCar/master/abcd.geojson'
).then(function(data) {
        var features=[]
        console.log({...data,features:[...features,data.features[0],data.features[1],data.features[2]]});
        
      return {...data,features:[...features,data.features[0]]};
    });
    this.setState({
      zips: data
    });
...
'https://envirocar.org/api/stable/measurements/'
        ...
        var features =[]
        console.log({...data,features:[...features,data.features[0],data.features[1],data.features[2]]});
      return {...data,features:[...features,data.features[0],data.features[1],data.features[2]]};
    });
    this.setState({
      zips: data
    });
